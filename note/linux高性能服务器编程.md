...
在某些特殊条件下，TCP连接的一端会向另一端发送带RST标志的报文段，即复位报文段。

### 访问不存在的端口

sudo tcpdump -i enx00e04c3613d6 -nt port 40000

![image-20220831213231162](linux高性能服务器编程.assets/image-20220831213231162.png)

telnet cloud113 40000

![image-20220831213205064](linux高性能服务器编程.assets/image-20220831213205064.png)

比如，我在112上用telnet 访问113的40000端口， 提示拒绝。就会返回一个复位报文。
...
TCP报文段所携带的应用数据按照长度分为两种：交互数据和成块数据。交互数据仅包含很少的字节，比如telnet，成块数据的长度则通常为TCP报文段允许的最大数据长度。
...
大端字节序是指一个整数的高位字节存储在内存的低位地址，低字节存储在内存的高位地址。[参考链接](https://blog.51cto.com/dabentu/1175806)

![image-20220902084451653](linux高性能服务器编程.assets/image-20220902084451653.png)

下面这段代码可以用来判断系统是大端字节序还是小端字节序。它是使用了一个union，里面有两个变量，value， union_bytes数组，这两个变量只能存在一个，它们都指向了一个内存地址，所以当代码里给value赋值的时候，通过union_bytes 就可以判断是大端存储还是小端存储了，01在union_bytes[0]存储的话就是大端存储。反之就是小端存储。debug下面这段程序可以看到给value赋值以后，union_bytes[0]存储的是02。所以是小端存储。

![image-20220902085341267](linux高性能服务器编程.assets/image-20220902085341267.png)
