...
第一代电脑的处理器是16位的英特尔的8088处理器，只能够管理1M的物理内存。因此，早期的电脑物理地址是从0x00000000开始到0x000FFFFF（1M）结束，而不是0xFFFFFFFF（这个是4G）。标记为“Low Memory”的 640KB 区域是早期 PC 可以使用的唯一随机存取存储器 (RAM)，实际上最早期的电脑仅仅可以使用16K、32K或者64K的随机存储（RAM）。
